...
import { ArrowLeft, Calendar, FileText, Package, AlertTriangle, CheckCircle, Activity, ClipboardCheck } from "lucide-react";
import { formatDate } from "@/lib/utils";
import { H1, H3, SmallText, Body } from "@/components/design-system/atoms/Typography";
import { Button } from "@/components/design-system/atoms/Button";
import { Badge } from "@/components/design-system/atoms/Badge";
import { Card } from "@/components/design-system/atoms/Card";
import { Accounting, Label } from "@/components/design-system";
...
export default function SRVDetailPage() {
    ...
    if (!srv) return (
        <div className="p-32 flex flex-col items-center justify-center gap-6">
            ...
            <Button variant="ghost" onClick={() => router.back()}>Return to List</Button>
        </div>
    );
    ...
    const itemColumns: Column<any>[] = [
        { key: "div_code", label: "DIV", align: "center", width: "5%" },
        {
            ...
            render: (_v, row) => (
                <div className="text-center">
                    <div className="font-medium text-slate-950">ITEM-{row.po_item_no}</div>
        ...
        { key: "pmir_no", label: "PMIR", width: "8%" },
        {
            ...
            render: (_v, row) => (
                <div>
                    <div className="font-medium text-slate-950">{row.challan_no || '-'}</div>
                    <SmallText className="text-slate-500">{row.challan_date ? formatDate(row.challan_date) : '-'}</SmallText>
                </div>
            )
        },
        {
            ...
            render: (_v, row) => (
                <div>
                    <div
                        ...
                        onClick={() => row.invoice_no && router.push(`/invoice/${encodeURIComponent(row.invoice_no.toString().trim())}`)}
                    >
                        {row.invoice_no || '-'}
                    </div>
                    <SmallText className="text-slate-500">{row.invoice_date ? formatDate(row.invoice_date) : '-'}</SmallText>
                </div>
            )
        },
        { key: "unit", label: "UNIT", align: "center", width: "6%" },
        { key: "order_qty", label: "ORDERED", align: "right", width: "8%", render: (v) => <Accounting>{v}</Accounting> },
        { key: "challan_qty", label: "CHALLAN", align: "right", width: "8%", render: (v) => <Accounting>{v}</Accounting> },
        { key: "received_qty", label: "RECEIVED", align: "right", width: "8%", render: (_v, row) => <Accounting className="font-medium text-[#1A3D7C]">{row.received_qty}</Accounting> },
        { key: "accepted_qty", label: "ACCEPTED", align: "right", width: "8%", render: (_v, row) => <Accounting className="font-medium text-[#16A34A]">{row.accepted_qty}</Accounting> },
        { key: "rejected_qty", label: "REJECTED", align: "right", width: "8%", render: (_v, row) => <Accounting className="font-medium text-[#DC2626]">{row.rejected_qty}</Accounting> },
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="p-6">
                    ...
                    <Label className="text-[10px] uppercase tracking-widest text-[#6B7280] mb-1">Receipt Volume</Label>
                    <div className="text-[28px] font-medium text-slate-950">
                        <Accounting>{totalReceived}</Accounting>
                    </div>
                    <SmallText className="text-[#1A3D7C] mt-2">Aggregated Units</SmallText>
                </Card>
                ...
                <Card className="p-6">
                    ...
                    <Label className="text-[10px] uppercase tracking-widest text-[#6B7280] mb-1">Quality Deficiency</Label>
                    <div className="text-[28px] font-medium text-[#DC2626]">
                        <Accounting className="text-[#DC2626]">{totalRejected}</Accounting>
                    </div>
                    <SmallText className="text-[#DC2626] mt-2">Impacted Inventory</SmallText>
                </Card>
                ...
                <Card className="p-6">
                    ...
                    <Label className="text-[10px] uppercase tracking-widest text-[#6B7280] mb-1">Verification Node</Label>
                    ...
                    <SmallText className="text-[#2BB7A0] mt-2">
                        {items[0]?.invoice_date ? `Finalized ${formatDate(items[0].invoice_date)}` : 'Awaiting Sync'}
                    </SmallText>
                </Card>
            </div>
