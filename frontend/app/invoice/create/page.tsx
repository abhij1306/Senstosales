...
import {
    Save, Loader2, AlertCircle, Search
} from "lucide-react";
    ...
    const loadDC = async (id: string) => {
        ...
        try {
            ...
            setFormData(prev => ({
                ...
                buyer_name: data.header.consignee_name || 'BHEL',
                buyer_gstin: data.header.consignee_gstin || '',
                buyer_address: data.header.consignee_address || '',
                buyer_state: (data.header as any).consignee_state || '',
    ...
    const topActions = (
        <div className="flex items-center gap-2">
            ...
            <Button
                ...
                size="sm"
                onClick={handleSave}
                disabled={saving || invoiceItems.length === 0 || isDuplicate || isChecking || !formData.invoice_number}
            >
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6">
                ...
                {!dcId && (
                    <Card className="p-6">
                        <Label className="text-[10px] uppercase tracking-widest text-slate-600 mb-2 block font-medium">Delivery Challan Reference</Label>
                ...
                {/* Invoice Details */}
                <Card className="p-6">
                    <H3 className="mb-6 font-medium text-slate-950 text-sm">Invoice Information</H3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">Invoice Number</Label>
                            <Input
                                ...
                                className={cn(
                                    ...
                                    isDuplicate ? "border-red-500 text-red-600 focus:ring-red-500/10" : "text-slate-930"
                                )}
                                placeholder="INV/001/24-25"
                            />
                            {isChecking && <SmallText className="text-slate-400 animate-pulse">Checking uniqueness...</SmallText>}
                            {isDuplicate && <SmallText className="text-red-500 font-medium">This number already exists in this FY</SmallText>}
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">Invoice Date</Label>
                            <Input type="date" value={formData.invoice_date} onChange={(e) => setFormData({ ...formData, invoice_date: e.target.value })} className="font-medium text-slate-930" />
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-400 font-medium">Linked DC</Label>
                            <div className="font-medium text-slate-950 tabular-nums px-3 py-2 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">{formData.dc_number || '---'}</div>
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-400 font-medium">Challan Date</Label>
                            <div className="font-medium text-slate-950 px-3 py-2 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">{formData.challan_date || '---'}</div>
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-400 font-medium">Order Reference</Label>
                            <div className="font-medium text-slate-950 px-3 py-2 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">{formData.buyers_order_no || '---'}</div>
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-400 font-medium">Order Date</Label>
                            <div className="font-medium text-slate-950 px-3 py-2 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">{formData.buyers_order_date || '---'}</div>
                        </div>
                    </div>
                </Card>
                ...
                {invoiceItems.length > 0 && (
                    <Card className="p-0 overflow-hidden border-none shadow-sm">
                        <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                            <H3 className="font-medium text-slate-950 text-sm">Invoice Items ({invoiceItems.length})</H3>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/30">
                                        <th className="py-3 px-6 text-left"><Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Lot</Label></th>
                                        <th className="py-3 px-6 text-left"><Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Description</Label></th>
                                        <th className="py-3 px-6 text-left"><Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">HSN</Label></th>
                                        ...
                                        <th className="py-3 px-6 text-right"><Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Total</Label></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {invoiceItems.map((item, idx) => (
                                        <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/10 transition-colors">
                                            <td className="py-4 px-6"><Accounting className="font-medium text-slate-930">{item.lotNumber}</Accounting></td>
                                            <td className="py-4 px-6 font-medium text-slate-950">{item.description}</td>
                                            <td className="py-4 px-6 text-[11px] font-mono text-slate-500">{item.hsnCode}</td>
                                            <td className="py-4 px-6 text-right"><Accounting className="text-slate-600">{item.quantity}</Accounting></td>
                                            <td className="py-4 px-6 text-right text-slate-600 font-mono text-[12px]">{formatIndianCurrency(item.rate)}</td>
                                            <td className="py-4 px-6 text-right font-medium text-slate-950"><Accounting isCurrency>{item.totalAmount}</Accounting></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="bg-slate-50/50 border-t border-slate-100 p-8">
                            <div className="grid grid-cols-2 gap-x-12 gap-y-4 max-w-sm ml-auto">
                                <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Taxable Value</Label>
                                <Accounting isCurrency className="text-right text-slate-600 font-medium">{formData.taxable_value || 0}</Accounting>

                                <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">CGST @ 9%</Label>
                                <Accounting isCurrency className="text-right text-slate-600 font-medium">{formData.cgst || 0}</Accounting>

                                <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">SGST @ 9%</Label>
                                <Accounting isCurrency className="text-right text-slate-600 font-medium">{formData.sgst || 0}</Accounting>

                                <div className="col-span-2 my-2 border-t border-slate-200"></div>

                                <Label className="text-[11px] uppercase tracking-[0.2em] text-slate-900 font-bold">Grand Total</Label>
                                <Accounting isCurrency className="text-right text-xl font-medium text-slate-950">{formData.total_invoice_value}</Accounting>
...
export default function CreateInvoicePage() {
    return (
        <Suspense fallback={<div className="p-32 text-center"><Body className="text-slate-400 animate-pulse">Loading template...</Body></div>}>
