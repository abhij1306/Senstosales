...
import {
    Save, Search, AlertCircle, Loader2, Plus, Trash2
} from 'lucide-react';
...
function CreateDCPageContent() {
    ...
    const topActions = (
        <div className="flex items-center gap-2">
            ...
            <Button color="primary" size="sm" onClick={handleSubmit} disabled={isSubmitting || items.length === 0 || isDuplicate || isChecking || !formData.dc_number}>
    ...
    return (
        ...
        >
            ...
            <div className="space-y-6">
                ...
                {!initialPoNumber && (
                    <Card className="p-6">
                        <Label className="text-[10px] uppercase tracking-widest text-slate-600 mb-2 block font-medium">Purchase Order Reference</Label>
                        ...
                        {poData && (
                            <div className="mt-4 p-4 bg-slate-50/50 rounded-xl border border-slate-100 flex items-center justify-between">
                                <div className="flex flex-col">
                                    <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Supplier</Label>
                                    <Body className="text-slate-950 font-medium">{poData.supplier_name}</Body>
                                </div>
                ...
                {/* Challan Details */}
                <Card className="p-6">
                    <H3 className="mb-6 font-medium text-slate-950">Challan Information</H3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">DC Number</Label>
                            <Input
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">DC Date</Label>
                            <Input
                                type="date"
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">Supplier Phone</Label>
                            <Input
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">Supplier GSTIN</Label>
                            <Input
                ...
                {items.length > 0 && (
                    <Card className="p-0 overflow-hidden border-none shadow-sm">
                        <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                            <H3 className="font-medium text-slate-950 text-sm">Dispatch Items ({items.length})</H3>
                        </div>
                                ...
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/30">
                                        <th className="py-3 px-6 text-left">
                                            <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Lot</Label>
                                        </th>
                                        <th className="py-3 px-6 text-left">
                                            <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Description</Label>
                                        </th>
                                        <th className="py-3 px-6 text-right">
                                            <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Ord</Label>
                                        </th>
                                        <th className="py-3 px-6 text-right">
                                            <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Bal</Label>
                                        </th>
                                        <th className="py-3 px-6 text-right">
                                            <Label className="text-[10px] uppercase tracking-widest text-slate-500 font-medium">Dlv Qty</Label>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {items.map((item, idx) => (
                                        <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/20 transition-colors">
                                            <td className="py-4 px-6">
                                                <Accounting className="text-slate-930 font-medium">{item.lot_no}</Accounting>
                                            </td>
                                            <td className="py-4 px-6">
                                                <div className="flex flex-col">
                                                    <Body className="text-slate-950 font-medium leading-normal">{item.description}</Body>
                                                    {item.drg_no && (
                                            ...
                                            <td className="py-4 px-6 text-right">
                                                <Accounting className="text-slate-600">{item.ordered_quantity}</Accounting>
                                            </td>
                                            <td className="py-4 px-6 text-right">
                                                <Accounting className="text-slate-600">{item.remaining_post_dc}</Accounting>
                                            </td>
                                            <td className="py-4 px-6">
                                                <Input
                                                    type="number"
                ...
                {/* Notes */}
                <Card className="p-6">
                    <Label className="text-[10px] uppercase tracking-widest text-slate-600 mb-4 block font-medium">Additional Notes</Label>
                        ...
                        {notes.map((note, idx) => (
                            <div key={idx} className="flex gap-2">
                                <Input
                                    ...
                                    className="font-medium text-slate-930"
                                />
                                <Button variant="ghost" size="sm" onClick={() => setNotes(notes.filter((_, i) => i !== idx))} className="text-slate-400 hover:text-red-600">
                                    <Trash2 size={16} />
                                </Button>
                            </div>
                        ))}
                        <Button variant="outline" size="sm" onClick={() => setNotes([...notes, ''])} className="border-dashed border-2 hover:border-slate-300">
