...
export function ReadinessStrip({ steps, className }: ReadinessStripProps) {
    return (
        <div className={cn("w-full bg-surface-1 border-b border-border", className)}>
                        ...
                        <React.Fragment key={idx}>
                            ...
                            >
                                ...
                                <div className="flex flex-col leading-none">
                                    ...
                                    {step.date && <span className="text-[10px] opacity-80 mt-1">{step.date}</span>}
                                </div>
                            </div>
                                ...
                                <div className="flex-1 h-[2px] mx-4 bg-border/60 relative">
                                    <div
                                        className={cn(
                                            ...
                                            step.status === 'completed' ? "bg-success/40" : "bg-transparent"
                                        )}
                                    />
                                </div>
                            )}
...
function StatusIcon({ status }: { status: ReadinessStep["status"] }) {
    switch (status) {
        ...
        case "current":
            return <div className="w-5 h-5 rounded-full border-2 border-current flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-current animate-pulse" /></div>;
        default:
