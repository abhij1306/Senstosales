    ...
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden flex">
        <div
          ...
          style={{
            width: `${(received / Math.max(ordered, delivered, 1)) * 100}%`,
          }}
        />
        <div
          className="h-full bg-blue-400 transition-all duration-500"
          style={{
            width: `${((delivered - received) / Math.max(ordered, delivered, 1)) * 100}%`,
          }}
        />
        {isDeliverPending && (
          <div
            ...
            style={{
              width: `${((ordered - delivered) / Math.max(ordered, delivered, 1)) * 100}%`,
            }}
          />
        )}
      </div>
      ...
      {(isOverDelivered || isReceivePending) && (
        <div className="flex items-center gap-1 text-[9px] font-medium animate-pulse">
          <AlertCircle
            size={10}
            className={isOverDelivered ? "text-red-500" : "text-amber-500"}
          />
