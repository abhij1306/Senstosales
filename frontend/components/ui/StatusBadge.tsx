...
const getVariant = (status: string, explicitVariant?: string) => {
    ...
    if (['rejected', 'failed', 'cancelled', 'overdue', 'missing'].includes(lower)) return 'error';
...
const StatusBadge = ({ status, variant, className }: StatusBadgeProps) => {
    ...
    const styles = {
        ...
        warning: "bg-[hsl(var(--warning)/0.15)] text-[hsl(var(--warning))] border-[hsl(var(--warning)/0.2)]",
        error: "bg-[hsl(var(--error)/0.15)] text-[hsl(var(--error))] border-[hsl(var(--error)/0.2)]",
