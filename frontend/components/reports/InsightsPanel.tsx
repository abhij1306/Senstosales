...
import { AlertTriangle, FileText, TrendingDown, ArrowRight } from 'lucide-react';
...
export function InsightsPanel({ pendingItems, uninvoicedCount, topItems = [], onAction }: InsightsPanelProps) {
    return (
        <div className="space-y-3">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-semibold text-gray-800">Pending Breakdown</h3>
                </div>
                    ...
                    {pendingItems.map((item, idx) => (
                        <div key={idx}>
                            ...
                            <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                                <div
                                    className={`h-full rounded-full ${item.percentage > 50 ? 'bg-red-400' :
                                        item.percentage > 30 ? 'bg-amber-400' :
                                            'bg-green-400'
                                        }`}
                    ...
                    <div className="flex items-start gap-2 mb-2">
                        ...
                        <div className="flex-1">
                            <h4 className="text-sm font-semibold text-red-900">
                                {uninvoicedCount} Challan{uninvoicedCount > 1 ? 's' : ''} Pending Invoice
                            </h4>
                        ...
                        {topItems.slice(0, 3).map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between text-xs">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-medium text-gray-700 truncate max-w-[150px]" title={item.name}>
                                        {item.name}
                                    </span>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    {item.trend !== 0 && (
                                        <span className={`text-[10px] font-medium ${item.trend > 0 ? 'text-green-600' : 'text-red-600'}`}>
