...
export function UnifiedTable({ data, loading }: UnifiedTableProps) {
    ...
    if (loading) {
        return <div className="p-8 text-center text-gray-500">Loading operational data...</div>;
    }
                                ...
                                >
                                    <td className="px-6 py-4">
                                        <span className={`
                                            ...
                                            ${row.pending_qty === 0 ? 'bg-green-100 text-green-700' :
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-semibold text-gray-900">PO #{row.po_number}</div>
                                        <div className="text-xs text-gray-500">{row.po_date}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-gray-700 max-w-[240px] truncate" title={row.material_description}>
                                        ...
                                        <div className="flex flex-col items-center gap-1">
                                            ...
                                            <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{ width: `${Math.min((row.dispatched_qty / row.ord_qty) * 100, 100)}%` }}
                                                ></div>
                                            </div>
                                        </div>
                                    </td>
