...
def get_deleted_files():
    ...
    output = run_git_command(['git', 'diff', '--name-status', '--diff-filter=D', 'HEAD'])
    if not output:
        return []
...
def get_modified_files():
    ...
    output = run_git_command(['git', 'diff', '--name-status', '--diff-filter=M', 'HEAD'])
    if not output:
        return []
    return [line.split('\t', 1)[1] for line in output.split('\n') if line]
...
def main():
    ...
    if deleted_routers:
        critical_changes.append(f"⚠️  {len(deleted_routers)} routers deleted - verify imports")
    
    # New migrations
    new_migrations = [f for f in added if 'migrations/' in f and f.endswith('.sql')]
    if new_migrations:
        critical_changes.append(f"✓ {len(new_migrations)} new migrations detected (expected)")
