...
def extract_routes_from_router(file_path):
    ...
    for method, path in matches:
        routes.append({"method": method.upper(), "path": path, "file": file_path.name})
...
def extract_api_calls_from_frontend(file_path):
    ...
    for match in matches:
        # Clean up the match
        path = match.strip()
        api_calls.append({"path": path, "file": file_path.name})
...
def main():
    ...
    for file in sorted(routes_by_file.keys()):
        ...
        for route in routes_by_file[file]:
            print(
                f"  {route['method']:6s} /api/{file.replace('.py', '')}{route['path']}"
            )
